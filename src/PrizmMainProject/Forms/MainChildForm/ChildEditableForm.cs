...
namespace Prizm.Main.Forms.MainChildForm
{
    ...
    public class ChildEditableForm : ChildForm, IModifiable, IValidatable
    {
        ...
        {
            validatableView = this;
            ValidateFalse = this.validatableView.Validate();
            if (this.IsModified)
            {
                if (isAutoValidate)
                {
                    SaveDialog dlg = new SaveDialog();
                    dlg.validate = ValidateFalse;
